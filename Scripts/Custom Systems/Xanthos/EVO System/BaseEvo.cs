...
namespace Xanthos.Evo
{
	...
	public abstract class BaseEvo : BaseCreature, IEvoCreature
	{
		...
		public TimeSpan RemainingTerm
		{
			get
			{
				return ( DateTime.MinValue == m_DeliveryDate ? m_InitialTerm : m_DeliveryDate.Subtract( DateTime.UtcNow ));
			}
		}
		...
		public bool Pregnant
		{
			...
			set
			{
				if ( !( this is IEvoGuardian ) && ( m_Pregnant = Blessed = value ) )
				{
					...
					DeliveryDate = DateTime.UtcNow + m_PregnancyTimer.Delay;
		...
		public override void OnThink()
		{
			...
			else if ( m_AlwaysHappy && DateTime.UtcNow >= m_NextHappyTime && null != ControlMaster && ControlMaster.Map == Map )
			{
				Loyalty = BaseCreature.MaxLoyalty;
				m_NextHappyTime = DateTime.UtcNow + TimeSpan.FromMinutes( 30.0 );
			}
		}
		...
		public void LoadSpecValues()
		{
			...
			if ( null != spec && null != spec.Stages )
			{
				...
				if ( null != stage )
				{
					...
					m_NextHappyTime = DateTime.UtcNow;
					m_MaxTrainingStage = spec.MaxTrainingStage;
					m_CanAttackPlayers = spec.CanAttackPlayers;
				}
			}
